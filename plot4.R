...
dev.set(4) # PNG Device
par(mfrow = c(2,2)) #Important: You need to call par() after setting the device, otherwise only 
                    #the last graph is plotted (overwrites other graphs) onto PNG Device


with(data_lean, 
     plot(data_lean$DateTime, data_lean$Global_active_power, type="l", 
          xlab = "", ylab = "Global Active Power"))#, cex.lab = 0.75, cex.axis = 0.75))


with(data_lean, 
     plot(data_lean$DateTime, data_lean$Voltage, type = "l", 
...
legend("topright", 
       ...
       lty = c(1,1), lwd = c(1,1), 
       bty = "n", #Since 4th plot has no "box" for the legend
       col = c("black", "red", "blue"))#, cex = 0.67))



with(data_lean, 
     plot(data_lean$DateTime, data_lean$Global_reactive_power, type = "l", 
          xlab = "datetime", ylab = "Global_reactive_power"))


dev.off()
#if (dev.cur() > 1 ) dev.off() #For good measure
